// Vendored min2phase — Kociemba two-phase algorithm for 3x3 Rubik's Cubes.
// Original: https://github.com/cs0x7f/min2phase/tree/4d183b9eff8119cac72bc50ef35a7d8990740e06 (MIT license)
//
// Modifications from upstream:
// - Removed random_cube, from_moves, apply_moves, random_moves (not needed)
// - Added AtomicBool cancel flag with checkpoints in phase1/phase2
// - Changed public solve() to return Result<String, String>
// - Changed from_facelet to accept &str

use lazy_static::lazy_static;
use std::sync::atomic::{AtomicBool, Ordering};

const CANCELLED: i8 = i8::MAX;

/// Set to true to cancel an in-progress solve. Checked periodically by the search.
...
impl Cubie {
    ...
    fn inv(src: &Cubie, inv: &mut Cubie) {
        ...
        for cn in 0..8 {
            inv.ca[(src.ca[cn] & 0x7) as usize] =
                cn as u8 | (((0x20 >> (src.ca[cn] >> 3)) & 0x18) as u8);
        }
    }

    fn from_facelet(&mut self, facelet: &str) -> i32 {
        if facelet.len() < 54 {
            return -1;
        }
        ...
        for i in 0..8 {
            ori = 0;
            while ori < 3 {
                if f[CORNER_FACELET[i][ori] as usize] == 0
                    || f[CORNER_FACELET[i][ori] as usize] == 3
                {
                    break;
                }
                ori += 1
...
// ---------------------------------------------------------------------------
// Static lookup tables (generated once on first use)
// ---------------------------------------------------------------------------
    ...
    fn init(&mut self) {
        ...
        for i in 0..10 {
            ...
            for j in 0..10 {
                ...
                self.canon_masks2[i] |= if (ix == jx) || ((ix % 3 == jx % 3) && (ix >= jx)) {
                    1
                } else {
                    0
                } << j;
...
fn init_sym2raw(
    ...
    for i in 0..n_raw {
        ...
        for s in (0..16).step_by(sym_inc) {
            if coord == 1 {
                Cubie::corn_mult(
                    &sctx.symcube[sctx.symmuli[0][s as usize] as usize],
                    &c,
                    &mut e,
                );
...
fn init_raw_sym_prun(
    ...
    let sym_e2c_magic: usize = if (prun_flag >> 4) & 1 == 1 {
        0x00DDDD00
    } else {
        0
    };
    ...
    let n_moves: usize = if is_phase2 != 0 {
        N_MOVES_P2
    } else {
        N_MOVES_P1
    };
...
impl StaticTables {
    ...
    fn init(&mut self, sctx: &StaticContext) {
        init_sym2raw(
            ...
            N_FLIP,
            0,
            &mut self.flip_sym2raw,
            &mut self.flip_raw2sym,
            &mut self.flip_selfsym,
        );
        init_sym2raw(
            sctx,
            N_TWST,
            1,
            &mut self.twst_sym2raw,
            &mut self.twst_raw2sym,
            &mut self.twst_selfsym,
        );
        init_sym2raw(
            sctx,
            N_PERM,
            2,
            &mut self.eperm_sym2raw,
            &mut self.eperm_raw2sym,
            &mut self.eperm_selfsym,
        );
        init_move_tables(sctx, self);
        init_raw_sym_prun(
            &mut self.slice_twst_prun,
            &self.slice_move,
            &self.slice_conj,
            &self.twst_move,
            &self.twst_selfsym,
...
impl Coord {
    ...
    fn from_cubie(&mut self, stbl: &StaticTables, src: &Cubie) -> i8 {
        ...
        self.prun = std::cmp::max(
            get_pruning(
                &stbl.slice_twst_prun,
                self.twst as usize * N_SLICE
                    + stbl.slice_conj[(self.slice * 8 + self.tsym) as usize] as usize,
            ),
            get_pruning(
                &stbl.slice_flip_prun,
                self.flip as usize * N_SLICE
                    + stbl.slice_conj[(self.slice * 8 + self.fsym) as usize] as usize,
            ),
        ) as i8;
        self.prun
    }
    ...
    fn move_prun(
        ...
        self.flip = stbl.flip_move
            [src.flip as usize * N_MOVES_P1 + sctx.symmove[mv][src.fsym as usize] as usize];
        self.fsym = (self.flip & 7) ^ src.fsym;
        self.flip >>= 3;
        self.twst = stbl.twst_move
            [src.twst as usize * N_MOVES_P1 + sctx.symmove[mv][src.tsym as usize] as usize];
        ...
        std::cmp::max(
            ...
            std::cmp::max(
                ...
                get_pruning(
                    ...
                    self.corn as usize * N_MPERM
                        + stbl.mperm_conj[self.mid as usize * 16 + self.csym as usize] as usize,
                ),
            ),
        ) as i8
    }
...
impl IdaContext {
    ...
    pub fn solve_cubie(
        ...
        for length1 in 0..21 {
            ...
            self.max_depth2 = std::cmp::min(
                MAX_DEPTH2 as i8,
                self.target_length as i8 - self.length1 as i8 - 1,
            );
            ...
            for urf_idx in 0..6 {
                ...
                if ret == 0 {
                    ...
                    println!(
                        "solution found in {:2}+{:2} moves urf={} premv={} probe={:5}: {}",
                        self.length1,
                        self.solution.length - self.length1,
                        self.solution.urf_idx,
                        self.solution.premv_len,
                        self.probes,
                        solbuf
                    );
                    return Ok(solbuf);
                } else if ret == CANCELLED as i32 {
                    return Err("cancelled".to_string());
                }
            }
        }
    ...
    fn init_phase2(&mut self, sctx: &StaticContext, stbl: &StaticTables) -> i8 {
        ...
        for i in self.valid1 as usize..self.depth1 as usize {
            Cubie::corn_mult(
                &self.p1_cubies[i],
                &sctx.movecube[self.mv[i] as usize],
                &mut cc,
            );
            Cubie::edge_mult(
                &self.p1_cubies[i],
        ...
        while depth2 >= prun {
            ...
            if sol_src == 1 {
                self.solution
                    .append_move(self.premv[self.premv_len as usize - 1] / 3 * 3 + 1);
            }
        ...
        if depth2 != self.max_depth2 {
            self.max_depth2 = std::cmp::min(
                MAX_DEPTH2 as i8,
                self.target_length as i8 - self.length1 - 1,
            );
    ...
    fn phase2(
        ...
        for m in 0..N_MOVES_P2 {
            ...
            nodex.corn = stbl.cperm_move
                [node.corn as usize * N_MOVES_P2 + sctx.symmove2[m][node.csym as usize] as usize];
            nodex.csym = sctx.symmult[nodex.corn as usize & 0xf][node.csym as usize] as u16;
            nodex.corn = nodex.corn >> 4;
            nodex.edge = stbl.eperm_move
...
// ---------------------------------------------------------------------------
// Lazy-initialized global tables
// ---------------------------------------------------------------------------
...
/// Solve a Rubik's cube represented as a facelet string.
///
/// Facelet format: 54 characters in U-R-F-D-L-B order (9 per face).
/// Solved cube: `UUUUUUUUURRRRRRRRRFFFFFFFFFDDDDDDDDDLLLLLLLLLBBBBBBBBB`
///
/// Returns space-separated moves (e.g. "R U F' D2 ...") or an error string.
pub fn solve(facelet: &str, maxl: u8) -> Result<String, String> {
    // Reset cancel flag at start of solve
    CANCEL_FLAG.store(false, Ordering::Relaxed);

    let mut cc = Cubie::new();
    if cc.from_facelet(facelet) < 0 {
        return Err("invalid facelet".to_string());
    }
    let verify = cc.verify();
    if verify < 0 {
        return Err(format!("invalid cube state: error code {}", -verify));
    }
