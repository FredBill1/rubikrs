...
impl RenderDevice {
    ...
    #[cfg(target_arch = "wasm32")]
    pub(crate) fn new_with_queue(device: WgpuWrapper<wgpu::Device>, queue: RenderQueue) -> Self {
        Self {
            device,
            queue: Some(queue),
        }
    ...
    pub fn create_buffer_with_data(&self, desc: &wgpu::util::BufferInitDescriptor) -> Buffer {
        #[cfg(target_arch = "wasm32")]
        if let Some(render_queue) = self.queue.as_ref() {
            // Avoid `mappedAtCreation` on WebGPU, which fails on some Android Chrome
            // implementations. Initialize with `Queue::write_buffer` instead.
            if desc.contents.is_empty() {
                ...
                return Buffer::from(wgpu_buffer);
            }
            ...
            if padded_size == unpadded_size {
                render_queue.write_buffer(&wgpu_buffer, 0, desc.contents);
            } else {
                let mut padded_contents = vec![0; padded_size as usize];
                padded_contents[..desc.contents.len()].copy_from_slice(desc.contents);
                render_queue.write_buffer(&wgpu_buffer, 0, &padded_contents);
            }
