...
- **NxN solver routing.** `rubik-solver::solve` routes N=4..8 through `crates/rubik-nxn-solver`; keep that path verified and do not silently fall back to `rcube-rs` for those orders.
