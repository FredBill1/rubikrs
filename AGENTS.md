...
- **Bevy is intentionally trimmed for web delivery.** The workspace enables both `webgl2` and `webgpu` so the browser picks the best available render backend automatically.
